...
public class CoachMarkiOS extends DriverManager {
    ...
    public void TC_037() throws Exception {
        ...
        try{
            ...
            LogUtil.warning("There is no 'skip' button visible in dashboard.");
            throw new RuntimeException(e.getMessage());
        }

        WebElement BackBtn = null;
        try{
            BackBtn=wait.until(ExpectedConditions.visibilityOfElementLocated(AppiumBy.iOSClassChain("**/XCUIElementTypeStaticText[`name == \"Back\"`]")));
            LogUtil.info("Skip button is available or not? : " + BackBtn.isDisplayed());
        } catch (Exception e) {
            LogUtil.warning("There is no 'skip' button visible in dashboard.");
            throw new RuntimeException(e.getMessage());
        }
...
//        try {
//            FourthTextElement = wait.until(ExpectedConditions.visibilityOfElementLocated(AppiumBy.iOSClassChain("**/XCUIElementTypeOther[`name == \"Line Chart. 1 dataset. \"`][2]")));
//            LogUtil.info("DashBoard fourth coach mark Text: \n " + FourthTextElement.isDisplayed());
//        } catch (Exception e) {
...
//        }
        ...
        try {
            ...
            FinishBtn.click();
            LogUtil.info("Clicking the 'Finish' Button for the coach mark complete in dashboard.");
        } catch (Exception e) {
